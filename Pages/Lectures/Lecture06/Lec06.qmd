...
- $X$ is a **discrete random variable** (or just "$X$ is **discrete**) if $S_X$ contains distinct values (often integers)
- $X$ is a **continuous random variable** (or just "$X$ is **continuous**) if $S_X$ has an infinite amount of values within a range
:::
:::
:::
...
- We could go through the same steps we did before, when deriving the p.m.f. for three tosses, but that could end up being quite long with the more tosses we have. Instead, we can use the following formula: 
