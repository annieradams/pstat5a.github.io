...
::: {style="font-size: 30px"}
It's known that 5% of all widgets produced at a factory are defective. A quality control test is used to check for these defects. The test isn't perfect:

- It will correctly identify a defective widget 90% of the time.
- It will incorrectly flag a non-defective widget as defective 8% of the time.

If a widget is randomly selected and the test flags it as defective, what is the probability that it is actually defective?
:::
:::
:::
...
- Let A denote the event "the widget is actually defective"
- Let B denote the event, "the widget is flagged as defective"


- Our goal is to find $\mathbb{P}(A \mid B)$, which the probability of the widget being defective given that the test flags it as defective. 
...
- Since $5\%$ of all widgets are defective, we know $$ \mathbb{P}(A) = 0.5 $$

- The probability of a widget not being defective is the complement of A,$$ \mathbb{P}(A ^\complement) = 1 - \mathbb{P}(A) = 1 - 0.05 = .95 $$

- The test correctly identifies a defective widget 90% of the time. This tells us $\mathbb{P}(B \mid A) = 0.9$

- Additionally, we are told that there is an $8\%$ chance that the widget is flagged as defective, when it is actually not defective. This gives us  $\mathbb{P}(B \mid A^\complement) = 0.08$
...
- Recall that we seek $\mathbb{P}(A \mid B)$.
- But, we only have information on $\mathbb{P}(B \mid A)$.
...
- The probability that the widget is actually defective give it was flagged defective by the test is approximately 37.2%. 
