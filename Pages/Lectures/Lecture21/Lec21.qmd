...
- The goal of **hypothesis testing** is to use data (in the form of a representative sample taken from the population), and determine whether or not this data supports our null or alternative hypothesis. 
...
- We have discussed hypothesis testing for a population *proportion* _p_, as well as how to perform hypothesis testing on a population *mean* $\mu$ .
...
- The _p_-value is the probability of observing something as or more extreme (in the direction of the alternative) than what we currently observe.

- As such, _p_-values that are smaller than the level of significance lead us to reject our null hypothesisl; i.e. we reject whenever $p < \alpha$.
...
d. Conduct the test, and phrase your conclusions in the context of the problem.
...
b. Since we do not have access to the population standard deviation, we use
...
c. From the _t_-table provided on the website (which will also be provided to you during the exam), the critical value is $\boxed{2.03}$. \

d. Since $|\mathrm{TS}| = |1.97| = 1.97 < 2.03$, we fail to reject the null:
...
- If we are conducting a two-sided hypothesis test, then both large *positive* values *and* large *negative* values of our test statistic would lead us to reject the null.  
