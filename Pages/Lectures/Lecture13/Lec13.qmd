...
$$ \mathbb{P}(|\mathrm{TS}| > c) = \alpha $$ if the null hypothesis is true.
We reject $H_0$ whenever $\overline{X}$ is far away from $\mu_0$) meaning $c$ will be the $(\alpha /2) \times 100\%$ percentile, scaled by negative 1, of either the standard normal distribution or the $t_{n - 1}$ distribution.
...
- With an $\alpha$ = .01, we can consult our t table and see that a one tailed test with $\alpha = 0.5$ and $dof$ = 39 leaves us with a a critical value of 2.43. 

- This means we reject the null whenever $\mathrm{TS} > 2.43$.
...
- Again, remember the intuition behind *why* this is: if we observe a value of $\widehat{P}$ that is much smaller than $p_0$, this leads us to the claim that $p < p_0$ (i.e. this leads us towards our *alternative* and *away* from our null).
...
Imagine you suspect a coin is rigged to land on heads more often so you conduct the following test: 

- $H_0$: The coin is fair
- $H_A$: The coin is rigged

:::{.fragment}
You flip the coin 10 times and get 9 heads. The p value asks us "If this coin is really fair, what is the probability I get 9 heads just by bad luck?" The answer is very small! - only about a 1% chance.  We can confirm this with $$ \mathbb{P} (X = k) = \binom{n}{k} \cdot p^k \cdot (1-p)^{n-k} = 10 \cdot .0019 \cdot 0.5 = .01  $$
:::

:::{.fragment}
So the _p_ value is .01. This means that the coin really is fair and you got unlucky (1% chance), or the coin is probably rigged. 
:::
...
The pink shaded area represents the p value. It is the area under the curve that is equivalent to or less than the test statistic. 
...
The pink shaded area represents the p value. It is the area under the curve that is equivalent to or greater than the test statistic. 
...
The pink shaded area represents the p value. It is the area under the curve that is equivalent to or more extreme than the test statistic in both directions. 


## How do we calculate our p value? 

Let's review this slide from Lecture 8: 
...
## How do we calculate our p value?{style="font-size:23px"}
...
- We found this area by looking at our z table! 
- To find $\mathbb{P}(X\leq .83)$ we broke it up as $0.83 = 0.8 + 0.03$ and then found the row on our z table that corresponds to $0.8$ and the column that corresponds to $.03$. 


## How do we calculate our p value?

- We find our p- value in the same way!
- Remember our p value is representing a probability - so to find it we are looking for an area under the curve. 
- We can look up our test statistic in the z table to find our corresponding probability. 
