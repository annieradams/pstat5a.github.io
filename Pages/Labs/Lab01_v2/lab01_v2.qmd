...
There are two main types of cells we will be using in this class: **Markdown cells** (which include text/descriptions, but no code) and **code cells** (which contain code that needs to be run). 
...
By the way, do you notice the little `In [1]:` at the left of our first cell? This is Jupyter's syntax for letting us know the order in which Jupyter is executing the code cells. The `1` in our cell from Task 2 above corresponds to the fact that this was the **1**^st^ code cell we executed in our document. 

## Coding with Python

There is a reason we use the word "language" to describe programming languages- that is because they function quite like a human language. This means that they have their own set of synatxes, and semantics. It is precisely these syntaxes and semantincs we will be learning over the course of these Computing Labs! \
...
It is important to note that all Python objects take up space in the form of memory. Nowadays, with the recent innovations in computers, this is not so much of an issue but historically, when many of these programming languages were first being created, optimizing space was of the utmost concern. (Even today, **efficiency** is a guiding tenent of most programmers!) \
...
Modules are Python files containing definitions for functions and classes. While data types and built-in functions in the Python standard library are available for immediate use, modules need to be imported (loaded) first. \
